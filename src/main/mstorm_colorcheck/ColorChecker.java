...
public class ColorChecker {
	...
	public ColorChecker() throws IOException {
		...
		try {
			...
			BufferedReader blue = new BufferedReader(new FileReader("ht2.gumi"));
			BufferedReader rdst = new BufferedReader(
					new FileReader("stop.gumi"));
			BufferedReader rdch = new BufferedReader(new FileReader(
					"choice.gumi"));
			...
			for (int i = 0; i < NB_COLORS; i++) {
				...
				blue_sample[i] = Float.parseFloat(blue.readLine());
			...
			epsilon = Math.abs(euclide_distance(line_sample, bg_sample))
					/ MARGIN;
	...
	public boolean isChoiceColor(float[] sample_to_check) throws Exception {
		...
		return (dist_color <= dist_line) && (dist_color <= dist_bg)
				&& (dist_color <= dist_st);
	}
	...
	public static float[][] insertionSort(float[][] array) {
		...
		for (int i = 1; i < array.length; i++) {
			...
			while (cpt >= 0
					&& (euclide_distance(array[cpt], line_sample) > euclide_distance(
							element, line_sample))) {
